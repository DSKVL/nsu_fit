...
size_t printScreen(char* buf, size_t bufSize) {
  int lineCount = 1;
  int charsCount = 0;
  size_t pos = 0;
...
void sigRead(int signo, siginfo_t *info, void* context) {
  (void) context;
  struct aiocb *req;
  ...
  if (aio_error(req)==0) {
    ...
    if (req->aio_fildes == socketd) {
        ...
        req->aio_buf = (void*) ((char*) req->aio_buf + READSIZE);
		    aio_read(req);
	    }
	  }
    if (req->aio_fildes == STDIN_FILENO) {
	    ...
	    if (input == ' ') {
	      ...
	      bufIndex = printScreen(buf, bufIndex);
