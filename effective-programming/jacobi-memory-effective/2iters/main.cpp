...
inline void countTwoIterations(float* Phi, float* PhiN, const size_t nX, const size_t nXArr, const size_t jArrLimit, 
  ...
  for (auto jArr = 2*nXArr; jArr < jArrLimit; jArr+=nXArr) {  
    ...
    for (auto i = 0ul; i < ((nX+1 - 8)&div8mask); i+=8) {
      auto topL    = _mm256_load_ps( PhiN + i - 1 + jArr - nXArr - nXArr);
      auto top     = _mm256_loadu_ps(PhiN + i +     jArr - nXArr - nXArr);
      auto topR    = _mm256_loadu_ps(PhiN + i + 1 + jArr - nXArr - nXArr);
      auto left    = _mm256_load_ps( PhiN + i - 1 + jArr - nXArr);
      auto vD      = _mm256_loadu_ps(D + i + jArr - nXArr);
      auto right   = _mm256_loadu_ps(PhiN + i + 1 + jArr - nXArr);
      auto bottomL = _mm256_load_ps( PhiN + i - 1 + jArr);
      auto bottom  = _mm256_loadu_ps(PhiN + i +     jArr);
      auto bottomR = _mm256_loadu_ps(PhiN + i + 1 + jArr);
      auto result0  = _mm256_fmadd_ps(vC, _mm256_add_ps(_mm256_add_ps(topL, topR),
                                                     _mm256_add_ps(bottomL, bottomR)), vD);
      auto result1 = _mm256_fmadd_ps(vA, _mm256_add_ps(top, bottom), result0);
      auto result  = _mm256_fmadd_ps(vB, _mm256_add_ps(left, right), result1);
       
      _mm256_storeu_ps(Phi + i + jArr - nXArr, result);
    }
    for (auto i = (nX+1)&div8mask; i < nX+1; i++) {
      Phi[i + jArr - nXArr] = A*(PhiN[i + jArr-nXArr-nXArr] + PhiN[i + jArr]) + B*(PhiN[i-1 + jArr-nXArr] + PhiN[i+1 + jArr-nXArr])
                     + C*(PhiN[i-1 + jArr-nXArr-nXArr] + PhiN[i-1 + jArr] + PhiN[i+1 + jArr-nXArr-nXArr] + PhiN[i+1 + jArr])
                     + D[i + jArr -nXArr];
    }
  }
}
