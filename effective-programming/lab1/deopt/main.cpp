...
#define CM(hX, hY) (1/(5/(hX*hX) + 5/(hY*hY)))
#define A(hX, hY) (CM(hX, hY)*0.5f*(5/(hX*hX) - 1/(hY*hY)))
#define B(hX, hY) (CM(hX, hY)*0.5f*(5/(hY*hY) - 1/(hX*hX)))
#define C(hX, hY) 0.05f
...
int main(int argc, char** argv) {
  ...
  auto deltas = std::vector<float>();
  auto start = std::chrono::high_resolution_clock::now();
  for (auto _ = 0ul; _ < nT; _++) {
    for (auto jArr = nXArr; jArr < jArrLimit; jArr+=nXArr)
      for (auto i = 1ul; i < nX + 1; i++) {
        PhiN[i + jArr] = A(hX,hY)*(Phi[i + jArr-nXArr] + Phi[i + jArr+nXArr]) + B(hX,hY)*(Phi[i-1 + jArr] + Phi[i+1 + jArr])
                       + C(hX,hY)*(Phi[i-1 + jArr-nXArr] + Phi[i-1 + jArr+nXArr] + Phi[i+1 + jArr-nXArr] + Phi[i+1 + jArr+nXArr])
                       + CM(hX, hY)*(2*rho[i + jArr] + 0.25f*(rho[i-1 + jArr] + rho[i+1 + jArr] + rho[i + jArr - nXArr] + rho[i + jArr + nXArr]));
      }

    std::transform(Phi,  Phi+arrSize,
                   PhiN, Phi,
                   [](const auto& a, const auto& b) { return std::abs(a-b); });
    std::cout << *std::max_element(Phi, Phi+arrSize) << "\n";
