...
private:
  ...
  inline void countIterations(float* Phi, float* PhiN, const size_t jStart, const size_t jLength) {
    for (auto it = 0ul; it < nT; it++) {

      if constexpr (rank != 0)
        while (conds[rank-1].last != it) {}
      countLine(Phi, PhiN, jStart*nXArr);
      conds[rank].first = conds[rank].first + 1;
      for (auto j = jStart + 1; j < jStart + jLength - 1; j++)
        countLine(Phi, PhiN, j*nXArr);

      if constexpr (rank != nThreads-1)
        while (conds[rank+1].first != it + 1) {}
      countLine(Phi, PhiN, (jStart + jLength - 1)*nXArr);
...
#ifndef NO_DELTA
      if constexpr (rank == 0)
        std::cout << std::reduce(deltas[it%nThreads].begin(), deltas[it%nThreads].end(), std::numeric_limits<float>::min(), 
                                 [](const auto& a, const auto& b){ return std::max(a, b); }) << "\n"; 
#endif
    }
  }
  //outer array for indicies, inner for certain thread.
  std::array<std::array<float, nThreads>, nThreads> deltas;
